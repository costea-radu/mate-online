...
// Butoanele sunt exportate și separat, pentru ecranele care au nevoie doar de
// unul singur (ex. sugestia „intră cu Google" de pe ecranul de confirmare).
const DiscordIcon = () => (
...
export function GoogleButton({ onClick, loading, disabled, label = 'Continuă cu Google' }) {
  return (
    <button type="button" onClick={onClick} disabled={loading || disabled}
      style={{ ...oauthBtnStyle, background:'#fff', color:'var(--text)', border:'1.5px solid var(--border)' }}>
      <GoogleIcon />
      {loading ? 'Se redirecționează...' : label}
    </button>
  );
}

export function DiscordButton({ onClick, loading, disabled, label = 'Continuă cu Discord' }) {
  return (
    <button type="button" onClick={onClick} disabled={loading || disabled}
...
export default function OAuthButtons({ onGoogle, onDiscord, googleLoading, discordLoading, busy }) {
  return (
    <>
        <GoogleButton onClick={onGoogle} loading={googleLoading} disabled={busy || discordLoading} />
        <DiscordButton onClick={onDiscord} loading={discordLoading} disabled={busy || googleLoading} />
