...
//
// GET /api/turneu?action=cron  (doar cron) → închide turneele expirate și dă
//   premiile pentru locurile 1-3.
// =====================================================================
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'cron') {
      ...
      return res.status(200).json(await turneu.finalizeExpired(supa));
    }
    ...
    if (action === 'list') {
      return res.status(200).json({ ok: true, profesor, ...await turneu.list(supa, userId, { isTeacher: profesor }) });
    }
