...
- Eroare „nu am obținut un document HTML complet (răspunsul s-a întrerupt)" →
  ...
  erori tranzitorii (rate-limit/suprasarcină, cu pauză de 15s) și re-cere
  strict documentul dacă modelul răspunde cu explicații; dacă eroarea
  persistă pe task-urile cu model de format MARE + Opus, încearcă Sonnet 5
  (mai rapid — generările uriașe cu Opus se pot apropia de limita de timp a
  funcției) sau un model de format mai mic. Mesajul include diagnosticul
  `[stop=…, continuări=…]`.
- Eroare „prompt is too long: … tokens > … maximum" → un PDF-sursă foarte
  DENS (ex. o culegere întreagă) depășește contextul modelului ca document
  nativ. Serverul reface acum cererea automat cu PDF-ul transformat în TEXT
...
- Eroare 504 / FUNCTION_INVOCATION_TIMEOUT sau „Șablonul rubricii e prea
  mare… [stop=max_tokens, continuări=0]" → generarea nu încăpea în limita
  funcției. De la 13 august: (1) șabloanele NU se mai regenerează integral
  (blocurile <style>/<script> nemodificate devin marcaje `data-tpl`
  reinserate de server); (2) limita funcțiilor e ridicată la **800s** în
  `vercel.json` (plan Pro, Fluid), iar generarea se oprește singură cu ~90s
  înainte de limită, cu eroare CLARĂ înregistrată. Notă: „▶️ Rulează acum" pe
  un test mare poate dura acum câteva minute — e normal. Dacă schimbi
  vreodată `maxDuration`, setează și env `FUNCTION_MAX_SECONDS` la aceeași
  valoare (Vercel → Settings → Environment Variables).
