...
- **Utilizatorul NU vede lei, ci CREDITE AI.** Conversia: `AI_CREDITS_PER_LEU` (implicit **100 credite = 1 leu de buget**), în `api/_lib/ai.js` (`CREDITS_PER_LEU`, `leiToCredits`, `fmtCredits`) și, identic, în `src/lib/aiCredit.js` pentru interfață. Deci pachetul de 10 lei se anunță „**+400 credite AI**", nu „+4 lei". Prețul rămâne în lei (e o plată reală); doar CE PRIMEȘTE e în credite. Aceeași unitate în numele produsului din Stripe (`create-checkout`) și în e-mailul de alertă (`stripe-webhook`). Dacă schimbi rata, schimb-o în ambele fișiere.
...
- **UI (pasul 2):** componenta `AILimite` afișează, în ordinea asta: (1) **consumul AI TOTAL** — o bară cu procentul, creditele folosite din total și câte îți mai rămân; (2) **cotele lunare pe funcții**, într-un **rolldown** (pliat implicit, starea se ține minte în `localStorage: ai_quotas_open`), cu mențiunea explicită că se **reportează și se completează între ele** — când una se termină, continui neîntrerupt din rezerva celorlalte; (3) **pachetele suplimentare**, cu ce primești scris în **credite AI**. Locul ei principal: **„Contul meu" (`/profil`), rolldown-ul „⚡ Consum AI" de sub cardul Abonament — pentru TOATE rolurile** (elev, profesor, părinte). Elevii o văd și sus în „📈 Progresul meu" (pagina Profesor Virtual). După plată, Stripe redirecționează către `/profil?topup=succes`, iar rolldown-ul se deschide singur cu confirmarea.
- `/api/ai-progress` întoarce `budget: { dayLei, monthLei, dayActions, monthActions, limits, effectiveMonthLei, creditsPerLeu, creditsUsed, creditsTotal, topup:{creditLei,credits,active,expiresAt,days}, packs (fiecare cu `credits`), features[], degraded, exempt }`. `null` până rulezi migrarea.
